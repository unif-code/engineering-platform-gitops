...
- `02`–`10` 各篇（今天的改动不触及它们）
- 新手册（子项目 B）——**2026-08-21 关闭，不做**。它只在 `2026-08-18` 与 `2026-08-19`
  两份 spec 的 Scope 里各出现一次（「新手册与 runbook 梳理（子项目 B/C）」），
  没有任何展开；用户复核时也不记得当初指什么，判断是误列。既然没有可恢复的需求，
  就不替它发明一个——凭空造一份与 runbook 重叠的文档，只会多一处必然漂移的副本。
  本条记录在此，避免它作为「未完成子项目」被继续往后带。
- 任何代码变更
