# PCS Candidate 2 And Runtime Evidence Reconciliation Implementation Plan
...
- docs 架构基线固定为 `2026-08-23.1`，事实提交为 `6267120f345e7ad967daf08fb244c6018054281d`。
- 事实采样时 GitOps main 为 `1c5034b9a9c29ab72fde63644c57fa88604c45b6`，frontend 为 `c392c6fc7a82a26f1eb4be22c35c6cda00e5d75c`，backend 为 `647d509bca1bbf9ff0f6ab719d5905d8f836e92f`。
- DEV Runtime 只能陈述最后一次成功观测：`2026-08-22 16:58 +08:00`；2026-08-23 Chrome 刷新为 `CANNOT VERIFY`，不得冒充新的服务器观测。
- frontend CI 产出的 OCI digest 为 `sha256:ee548974e159916ba7ca0fafe8bb30d72722a34625ffbce31d6e495324d06c0c`；`linux/amd64` platform manifest digest 与运行 Image ID 尚未核验，不得猜测。
...
- MinIO Server 继续保留原 index/amd64 digest，并保留“上游归档且早于最后 CVE 修复版本”的 `BLOCKED` 判定。
- frontend 行写入 Source Commit、tag `sha-c392c6f`、CI run `32549901199` 和 OCI digest `sha256:ee548974e159916ba7ca0fafe8bb30d72722a34625ffbce31d6e495324d06c0c`；状态为“OCI 制品已产生，platform manifest digest、运行 Image ID 与工作负载均未核验”。
...
部署后核验清单中，bootstrap 版本观察可标为完成；MinIO、双应用 platform digest、Flux Inventory、实际 Image ID 和全栈一致性全部保持未完成。

- [ ] **Step 4: 实现 `CURRENT_PCS`**
...
- [ ] **Step 6: 提交 Candidate 2 与 validator**
...
- [ ] **Step 1: 回填最新 bootstrap 证据和 Runtime 边界**
...
- [ ] **Step 4: 更新 image owner handoff**

把 `runbook/10-image-owner-handoff.md` 的检查时间改为 `2026-08-22`，状态保持 `BLOCKED`，并用以下事实替换旧代码骨架判断：

- frontend 当前 commit 有 Dockerfile、nginx 80 启动合同和 main image workflow；CI run `32549901199` 产生 tag `sha-c392c6f` 与 OCI digest `sha256:ee548974e159916ba7ca0fafe8bb30d72722a34625ffbce31d6e495324d06c0c`。
- frontend `linux/amd64` platform manifest digest 尚未从 Registry 单独核验，运行 Image ID 尚不存在。
...
Handoff 表填入上述确定值；不能确定的 platform manifest digest 和运行 Smoke 明确写 `NOT_VERIFIED` 或 `NOT_EXECUTED`，不留空、不猜测。

- [ ] **Step 5: 运行定向文档与 validator 检查**

Run:

```bash
rg -n 'bootstrap|BLOCKED|NOT_EXECUTED|NOT_VERIFIED|ee548974|647d509b' runbook/01-bootstrap.md runbook/06-apps.md runbook/10-image-owner-handoff.md pcs/candidate-2.md
...
- [ ] **Step 1: 切换 acceptance 到 Candidate 2**
...
3. `PENDING`：PG PITR 与 etcd restore 未执行。
4. `BLOCKED`：MinIO 供应链决定未关闭，Object Lock 未验证。
5. `BLOCKED`：Candidate 2 尚未与实际 Image ID/Chart revision 全量对齐。
6. `PENDING`：Metrics API、容量包络、Stop Gate 与整机重启自愈未形成证据。

- [ ] **Step 2: 更新 Stop Gates 和最终结论**

将 Docker/containerd 清退标为已完成；将 frontend OCI digest 标为已取得，但 backend digest、frontend platform manifest、实际 Image ID、稳定 DNS、MinIO、Metrics API 与应用 Runtime 保持未完成。
...
Expected: fast profile 134 tests PASS，GitOps manifests validated，静态检查通过，工作树干净。若本机 ShellCheck 不是 CI 固定的 0.9.0，最终报告必须明确本地版本，不能宣称完全复刻 CI。
...
Expected: `candidate-1` 只可能出现在明确的历史说明中；其余过期或非平台表述零命中。最新提交序列依次为 plan、Candidate 2、runbook、acceptance。
