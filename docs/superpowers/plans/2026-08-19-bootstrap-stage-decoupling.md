...
**另有若干处硬编码 `source "${script_dir}/lib/<name>.sh"` 字面量断言，必须一并改**
...
| `test_bootstrap.py` `ExecSafetyTest` | `lib/exec-safety.sh`、`lib/path-facts.sh` |

源码中每处均有注释指向本任务。漏改任一处，对应断言会在迁移后变红。
