...
readonly -a MUTATING_STAGES=(10 20 30 40 50 60 100 110 120 130 140 150 160 170)
...
check_result_is_complete() {
  case "$1:$2" in
    ...
    160:ALREADY_COMPLIANT|170:ALREADY_COMPLIANT) return 0 ;;
    *) return 1 ;;
  esac
}
...
check_result_requires_apply() {
  case "$1:$2" in
    ...
    160:PASS_BUSINESS_READY_EVIDENCE_CHECK|\
    170:PASS_OPENBAO_RUNTIME_CHECK) return 0 ;;
    *) return 1 ;;
  esac
}
...
apply_result_is_success() {
  case "$1:$2" in
    ...
    160:PASS_BUSINESS_READY|170:PASS_OPENBAO_RUNTIME_INSTALLED|\
