...
服务端中文提示会明确区分三个用途：`解封 OpenBao`（仅 sealed 时需要）、root 登录后的
`旧份额轮换授权`、后续 `recover-verify` 中的 `新份额验证`。提示中的 `第 N/3 份` 是当前
轮次的提交序号，不是要求输入文字 `shareN`；实际粘贴的是对应恢复包解密出的隐藏值。
只有服务器状态回读符合预期才显示 `N/3 已由服务器回读确认`，不显示明文或首尾字符。
旧份额授权达到 3/3 后进入新份额待验证状态，不要求第四份旧份额；中断后的重入按实时
状态续跑，不能将 root 再登录误认为必须从头重复所有份额。任何 `STOP_*` 后停止粘贴。

候选包流程输出 `OPENBAO_CANDIDATE_STEP=<固定步骤名> STATUS=START|PASS|FAIL`，只用于
定位失败边界。步骤覆盖临时目录准备、加密备份读取、源包/响应文件校验、摘要计算、
候选包规范化与发布及发布后回读；不输出参数、路径内容、原始响应、nonce 或密钥。
嵌套步骤失败会同时标记内层和外层 `FAIL`，排查时从第一个 `FAIL` 开始；`START` 后无
结果不等于成功。该诊断不改变原来的 `STOP_*`、退出码和清理门禁，也不构成自动重试授权。
